...
class DBUtils(object):
    ...
    def __init__(self, mission='Test', db_var=None, echo=False):
        ...
        self.Instrumentproductlink = None
        self.Filecodelink = None
    ...
    def addProcess(self,
        ...
        """
        assert (output_timebase in ['RUN', 'ORBIT', 'DAILY', 'WEEKLY', 'MONTHLY', 'YEARLY', 'FILE'])
    ...
    def _nameSubProduct(self, instr, product_id):
        ...
        # are there any replacements to do?  If not we are done
        match = False
        for r in repl:
            if r in instr:
                match = True
    ...
    def addFile(self,
        ...
        @keyword utc_stop_time: utc end time of the file
        @type utc_stop_time: datetime.datetime
    ...
    def getFileFullPath(self, filename):
        ...
        sq = self.session.query(self.File.filename, self.Product.relative_path).filter(
                self.File.file_id == filename).join(
                (self.Product, self.File.product_id == self.Product.product_id)).one()
    ...
    def getActiveInspectors(self):
        ...
        retval = [activeInspector(os.path.join(basedir, ans.relative_path, ans.filename), ans.arguments, ans.product)
                  for ans in sq]
        return retval
